...
- **Agent interop**: external LLM agents can talk to the wiki via `llmwiki` CLI commands with structured JSON output. The wiki becomes a shared knowledge layer, not a siloed tool.
...
1. **CLI tool (`llmwiki`)** — the foundation. Implement `init`, `ingest`, `query`, `lint`, `status` commands. TypeScript/Node.js. `--json` flag for agent interop. This unblocks everything else — the VS Code extension wraps it, GitHub Actions calls it, external agents invoke it.
...
- **Complex MCP server from day one** — start with CLI `--json` output. Add MCP server mode (`llmwiki mcp`) later when agent interop patterns stabilize.
